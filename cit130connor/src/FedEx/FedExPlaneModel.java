...
public class FedExPlaneModel {
    ...
    final static int MAX_PACKAGE_SIZE_WEIGHTGRAMPERINCH3 = 40;
    ...
    public CargoPlane generateCargoPlane(int crewSize, int parcelNum){
        ...
        plane.setParcels(generateParcelList(parcelNum));
        return plane;
    }
    
    /**
     * Generates a list the inputted size and 
    ...
    private List<CrewMember> generateCrewMembers(int crewSize){
     ...
     private List<Parcel> generateParcelList(int numPar){
        ...
        for (int i = 0; i <numPar; i++){
         Parcel p = new Parcel();
         p.setSizeCubeInches(rand.nextInt(MAX_PACKAGE_SIZE_INCH3));
